...
<body class="h-full bg-slate-50 text-slate-800 font-sans antialiased selection:bg-brand-500 selection:text-white"
    ...
    $roleBadgeColor = $isSuperAdmin ? 'bg-purple-100 text-purple-700 border-purple-200' : ($isOwner ? 'bg-blue-100
    text-blue-700 border-blue-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200');
    @endphp
    ...
    <div class="min-h-screen flex flex-col lg:flex-row bg-slate-50">
        ...
        <div x-show="isMobileOpen" x-transition:enter="transition-opacity ease-linear duration-200"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition-opacity ease-linear duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" @click="isMobileOpen = false"
            class="fixed inset-0 z-40 bg-slate-900/50 backdrop-blur-xs lg:hidden" style="display: none;">
        </div>

        <!-- ============================================================ -->
        <!-- TAILADMIN BACKEND SIDEBAR -->
        <!-- ============================================================ -->
        <aside :class="isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <div>
                <div class="h-16 flex items-center justify-between px-5 border-b border-slate-800/80">
                    <a href="{{ $isSuperAdmin ? route('superadmin.dashboard') : ($isOwner ? route('tenant.owner.dashboard', ['tenant' => $tenantSlug]) : route('tenant.participant.dashboard', ['tenant' => $tenantSlug])) }}"
                        ...
                        <div
                            class="w-9 h-9 rounded-xl bg-brand-500 flex items-center justify-center text-white font-bold text-lg shadow-md shadow-brand-500/20">
            ...
            <div class="p-4 border-t border-slate-800 bg-slate-950/50">
                <div class="flex items-center gap-3">
                    <div
                        class="w-8 h-8 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-brand-400 font-bold text-xs">
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-screen">
            ...
            <header
                class="h-16 bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 shadow-xs">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open"
                            ...
                            <div
                                class="w-8 h-8 rounded-lg bg-brand-500 text-white font-bold flex items-center justify-center text-sm shadow-xs">
                        ...
                        <div x-show="open" @click.outside="open = false"
                            ...
                            <div class="px-4 py-2 border-b border-slate-100">
                                ...
                                <span
                                    class="inline-block mt-1 px-2 py-0.5 rounded-md text-[10px] font-bold border {{ $roleBadgeColor }}">
                                    {{ $roleLabel }}
                                </span>
                            </div>
            ...
            <main class="p-4 sm:p-6 lg:p-8 flex-1 space-y-6">
                ...
                <div x-data="{ show: true }" x-show="show"
                    class="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-medium flex items-center justify-between shadow-xs">
                ...
                @if(session('error'))
                <div x-data="{ show: true }" x-show="show"
                    class="p-4 rounded-xl bg-red-50 border border-red-200 text-red-800 text-xs font-medium flex items-center justify-between shadow-xs">
                    <div class="flex items-center gap-2">
            ...
            <footer class="bg-white border-t border-slate-200 py-4 px-6 text-center text-xs text-slate-500">
                &copy; {{ date('Y') }} <span class="font-bold text-slate-700">MariLMS AI</span> — Multi-Tenant Learning
                Management System.
            </footer>
